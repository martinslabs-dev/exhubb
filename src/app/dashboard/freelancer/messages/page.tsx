...
export default async function FreelancerMessagesPage({
  ...
  const conversations = await prisma.conversation.findMany({
    ...
    include: {
      ...
      order:  { select: { id: true, gig: { select: { title: true } }, product: { select: { title: true } } } },
      messages: {
          ...
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6 py-12">
              ...
              <p className="text-xs text-gray-400 mb-4">
                Messages from clients will appear here once you receive gig orders.
              </p>
              <Link
            ...
            conversations.map((conv) => {
              ...
              const orderTitle = conv.order?.gig?.title ?? conv.order?.product?.title ?? "Order";
              ...
              return (
                <Link
                  key={conv.id}
                  href={`/dashboard/freelancer/messages?conv=${conv.id}`}
                  className={cn(
        ...
        {!active ? (
          ...
          <>
            ...
            <div className="h-14 px-5 border-b border-gray-100 flex items-center gap-3 flex-shrink-0">
              ...
              {active.order && (
                <Link
                  href={`/dashboard/freelancer/orders/${active.order.id}`}
