...
const STATUS_CONFIG = {
  ...
  SHIPPED:     { label: "Delivering",  color: "bg-purple-50 text-purple-700" },
...
export default async function FreelancerDashboard() {
  ...
  const [activeGigs, queueCount, revenue30d, completedCount, totalOrders, activeOrders, dbUser, ratingAgg] = await Promise.all([
    prisma.gig.count({ where: { freelancerId: userId, isActive: true } }),
    prisma.order.count({
      where: { sellerId: userId, status: { in: ["CONFIRMED", "IN_PROGRESS"] } },
    }),
    ...
    prisma.order.count({ where: { sellerId: userId, status: { notIn: ["CANCELLED", "REFUNDED"] } } }),
    prisma.order.findMany({
      where:   { sellerId: userId, status: { in: ["CONFIRMED", "IN_PROGRESS", "PENDING"] } },
      include: {
  ...
  const completionRate   = totalOrders > 0 ? Math.round((completedCount / totalOrders) * 100) : null;
  const hasProfile       = !!(dbUser?.bio || dbUser?.location);
  const hasGig           = activeGigs > 0;
  const hasCompletedOrder = completedCount > 0;

  const checklist = [
    { label: "Create your account",              done: true,              href: null                               },
    { label: "Complete your freelancer profile",  done: hasProfile,        href: "/dashboard/settings/profile"     },
    { label: "Create your first gig",            done: hasGig,            href: "/dashboard/freelancer/gigs/new"  },
    { label: "Set up your payout method",        done: false,             href: "/dashboard/settings/payouts"     },
    { label: "Complete your first order",        done: hasCompletedOrder, href: "/dashboard/freelancer/orders"    },
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Welcome back, {name}. Here&apos;s your service overview.
          </p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            sub: `${completedCount} of ${totalOrders} orders`,
            icon: BarChart2,
          },
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {activeOrders.length === 0 ? (
              <div className="px-5 py-12 text-center">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Create a gig to start receiving orders from clients.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left text-xs font-medium text-gray-400 px-5 py-3">Service</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {activeOrders.map((order) => {
                      ...
                      const title = order.gig?.title ?? "Order";
                      return (
          ...
          <div className="bg-white border border-gray-200 rounded-lg">
            ...
            <div className="divide-y divide-gray-50">
              {[
                { label: "Total orders received", value: String(totalOrders)    },
                { label: "Orders completed",      value: String(completedCount) },
                { label: "Completion rate",       value: completionRate !== null ? `${completionRate}%` : "—" },
                { label: "Orders in progress",    value: String(queueCount)     },
              ].map(({ label, value }) => (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg">
            ...
            <div className="divide-y divide-gray-50">
              {[
                { label: "Create a gig",        href: "/dashboard/freelancer/gigs/new",   icon: PlusCircle    },
                { label: "View active orders",  href: "/dashboard/freelancer/orders",      icon: ClipboardList },
                { label: "My gigs",             href: "/dashboard/freelancer/gigs",        icon: Briefcase     },
