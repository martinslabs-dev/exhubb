...
import { ArrowRight, CheckCircle2, DollarSign, Star, Package, type LucideIcon } from "lucide-react";

const BADGE_ICON_MAP: Record<string, LucideIcon> = { DollarSign, Star, Package };
  ...
  return (
    <section className="section-padding bg-gray-900 relative overflow-hidden">
      ...
      {floatingBadges.map((b, i) => {
        const BadgeIcon = BADGE_ICON_MAP[b.icon];
        return (
        <motion.div
          key={b.text}
        ...
        >
          <motion.div
            animate={{ y: [0, -8, 0] }}
            transition={{ duration: 2.8, repeat: Infinity, ease: "easeInOut", delay: b.delay }}
            className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 backdrop-blur-sm"
          >
      ...
      <div className="container-tight relative z-10 text-center">
        ...
        <motion.p
          ...
          viewport={{ once: true }}
          className="section-chip bg-primary-950/60 text-primary-400 border border-primary-800/40 mb-4 mx-auto w-fit"
        >
          Start Earning Today
        </motion.p>
        ...
        <motion.p
          ...
          className="text-gray-400 max-w-xl mx-auto mb-8 text-lg leading-relaxed"
        >
        ...
        <motion.div
          ...
          className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-w-xl mx-auto mb-10 text-left"
        >
          {perks.map((p, i) => (
            <motion.div
              key={p}
        ...
        >
          <motion.a
            ...
            className="flex items-center gap-2 bg-primary-600 hover:bg-primary-500 text-white font-bold px-7 py-3.5 rounded-full text-base shadow-lg shadow-primary-900/40 transition-all"
          >
          ...
          <motion.a
            ...
            className="flex items-center gap-2 border border-white/20 hover:border-primary-600/50 bg-white/5 hover:bg-primary-950/50 text-white font-semibold px-7 py-3.5 rounded-full text-base transition-all"
          >
            Offer Your Services
          </motion.a>
        </motion.div>
